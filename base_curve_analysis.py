...
def split_data(data, displacement_level):
    ...
    idx = np.where(data[:, 0] == displacement_level)
    split_disp = data[idx]
    max_trials = np.amax(split_disp[:,1])
    idxes =[]
    output=[]
    for i in range(0,int(max_trials)):
        idxes.append(np.where(split_disp[:, 1] == i+1))
        output.append(split_disp[idxes[i]])
        output[i][:,6] = output[i][:,6]-output[i][0,6]
...
if __name__ == "__main__":
    ...
    p = []
    color = pl.cm.jet(np.linspace(0,1,19))
    
    for j in range(0, 19):
        #filename = "basecurves_111318/base_curve" + str(j + 1) + "_ds.csv"
        filename = "basecurves_031419/" + str(j + 1) + ".csv"
        print(filename)
